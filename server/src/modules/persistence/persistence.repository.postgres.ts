...
const mapRecord = (row: RecordRow): SampleRecord => ({
  ...
  sampleType: String(row.sampleType ?? ""),
  samplingDate: String(row.samplingDate ?? ""),
  receptionDate: String(row.receptionDate ?? ""),
  testDate: String(row.testDate ?? ""),
  RPS4Y1: String(row.RPS4Y1 ?? ""),
  PKHD1L1: String(row.PKHD1L1 ?? ""),
  CRABP1: String(row.CRABP1 ?? ""),
  GAPDH: String(row.GAPDH ?? ""),
  ...
  async listRecords(params: {
    ...
    const keywordCondition = keyword
      ? or(
          ...
          ilike(recordsTable.sampleId, `%${keyword}%`)
        )
      : undefined;
    const condition = and(
