...
# -------- USB offline: bundled OpenSlide DLLs --------
_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
_OPENSLIDE_BIN = _PROJECT_ROOT / "assets" / "openslide" / "bin"
if _OPENSLIDE_BIN.is_dir():
    if hasattr(os, "add_dll_directory"):
        os.add_dll_directory(str(_OPENSLIDE_BIN))
    os.environ.setdefault("PATH", str(_OPENSLIDE_BIN) + os.pathsep + os.environ.get("PATH", ""))

# -------- HuggingFace mirror (China), set empty to use official --------
_HF_MIRROR = os.environ.get("HF_ENDPOINT", "https://hf-mirror.com")
if _HF_MIRROR:
    os.environ.setdefault("HF_ENDPOINT", _HF_MIRROR)

# -------- add CLAM-master to path --------
_CLAM_DIR = str(Path(__file__).resolve().parent / "CLAM-master")
...
parser.add_argument('--slide_ext', type=str, default= '.svs')# .svs
